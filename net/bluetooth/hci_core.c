...
int bt_get_sleep_monitor_cb(void *priv, unsigned int *raw_val,
		int check_level)
{
	int state = DEVICE_UNKNOWN, type = *raw_val;
	struct hci_dev *hdev = (struct hci_dev*)priv;

	*raw_val = hdev->dev_flags;

	if (type == SLEEP_MONITOR_CHECK_SOFT) {
		if (test_bit(HCI_INQUIRY, &hdev->flags))
			state = DEVICE_ON_ACTIVE2;
		else if (test_bit(HCI_ISCAN, &hdev->flags) ||
			test_bit(HCI_PSCAN, &hdev->flags) ||
			test_bit(HCI_ADVERTISING, &hdev->dev_flags))
	...
	pr_debug("%s: type[%d], state[%d], raw[%u]\n", __func__,
			type, state, *raw_val);
